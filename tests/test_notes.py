...
def _create_note(client, auth_headers, task_id: int):
    return client.post(f"/tasks/{task_id}/notes", json={"title": "Test note", "text": "Test note text"}, headers=auth_headers).json()
...
def test_note_creation(client, auth_headers):
    ...
    response = client.post(f"/tasks/{task_id}/notes", json={"title": "Test note", "text": "Test note text"}, headers=auth_headers)
...
def test_note_retrieval(client, auth_headers):
    workspace_id = _create_workspace(client, auth_headers).json()["workspace_id"]
    task_id = _create_task(client, auth_headers, workspace_id).json()["task_id"]
    note_id = _create_note(client, auth_headers, task_id)["note_id"]
...
def test_note_editing(client, auth_headers):
    ...
    response = client.patch(f"/notes/{note_id}", json={"title": "Edited note title"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
